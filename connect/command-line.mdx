...
the inbox backlog, filing, search coverage and the link counts, each with the one thing that
fixes it. It is the report described under [brain health](/brains/health#is-it-still-being-fed),
...
Pass `--ck` with a [service credential](/connect/service-credentials) to run `import`, `verify`, `upload` and `health` against that credential's brain without signing in, for example from a scheduled job. A credential is pinned to one brain, so `init`, `connect` and `personal` need a login.
