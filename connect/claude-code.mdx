...
Hand Claude Code a document and ask it to file it: it uploads the file to the brain's inbox first, gets the extracted text back from Cortex, writes the pages and archives the item, all with its own tokens. The file's bytes never pass through the conversation. With the [command line](/connect/command-line) installed, the upload streams from disk; without it, Claude Code uses a one-time upload address from its shell. [Ask your agent](/capture/asking-your-agent#giving-your-agent-a-file) has the steps.
...
## An assistant that knows your tasks

Once meetings and mail flow in, Claude Code can answer *"what have I committed to this week?"*,
move what you agree with onto your list, and push tasks to the tool you actually work from. See
[tasks](/filing/tasks).
